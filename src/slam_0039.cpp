...
// #include "g2o/types/sba/types_six_dof_expcamera.h"
...
{
  unsigned long frame_number = 394; //394;
  ...
  map.info.width = 500;
  map.info.height = 350;
  map.info.origin.position.x = -100;
  ...
  while (ros::ok())
  {
    ...
    if(j == 0){
      ...
      //if reliable
      if(std::abs(prev2cur_trans.x()) < 1.5 && std::abs(prev2cur_trans.y()) < 0.35 && std::abs(prev2cur_trans.z()) < 1.5){
    ...
    ros::spinOnce();
    camera_pose.publish(marker_cam);
    map_pub.publish(map);
